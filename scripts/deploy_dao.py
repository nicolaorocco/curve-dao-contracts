...
POOL_TOKENS = {
    "LOAD-EMN": ('0x845838DF265Dcd2c412A1Dc9e959c7d08537f8a2', 1),
}
...
def deploy_part_one(admin, confs=1, deployments_json=None):
    token = ERC20CRV.deploy("LOAD Token", "LOAD", 18, {'from': admin, 'required_confs': confs})
    voting_escrow = VotingEscrow.deploy(
        token, "Rewards-earning LOAD", "reLOAD", "reLOAD_1.0.0", {'from': admin, 'required_confs': confs}
    )
    deployments = {
...
def deploy_part_two(admin, token, voting_escrow, confs=1, deployments_json=None):
    ...
    minter = Minter.deploy(token, gauge_controller, config.TREASURY, config.FOUNDERS, {'from': admin, 'required_confs': confs})
