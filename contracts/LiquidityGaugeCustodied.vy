# @version 0.2.4
"""
@title Liquidity Gauge with Custodied Deposits
...
custodian: public(address)

balanceOf: public(HashMap[address, uint256])
custodianBalanceOf: public(HashMap[address, uint256])
...
@external
def __init__(lp_addr: address, _minter: address, _custodian: address):
    """
    ...
    assert _custodian != ZERO_ADDRESS

    self.lp_token = lp_addr
    self.minter = _minter
    self.custodian = _custodian
...
def user_checkpoint(addr: address) -> bool:
    ...
    _balance: uint256 = self.balanceOf[addr] + self.custodianBalanceOf[addr]
...
def deposit(_value: uint256, addr: address = msg.sender):
    ...
    if _value != 0:
        ...
        _balance += self.custodianBalanceOf[addr]
...
def withdraw(_value: uint256):
    ...
    _balance += self.custodianBalanceOf[msg.sender]
...
@external
def adjust_balances(_from: address, _to: address, _value: uint256) -> bool:
    """
    @notice Register a change in custodied balances
    @dev use `ZERO_ADDRESS` for `_from` or `_to` to indicate a deposit or withdrawal
    @param _from sender address
    @param _to receiver address
    @param _value Number of tokens being transferred within the custodian
    @return bool success
    """
    assert msg.sender == self.custodian
    ...
    # adjust the sender balance
    if _from != ZERO_ADDRESS:
        self._checkpoint(_from)
        _balance: uint256 = self.custodianBalanceOf[_from] - _value
        self.custodianBalanceOf[_from] = _balance
        self._update_liquidity_limit(_from, _balance, _supply)

    # adjust the receiver balance
    if _to != ZERO_ADDRESS:
        self._checkpoint(_to)
        _balance: uint256 = self.custodianBalanceOf[_to] + _value
        self.custodianBalanceOf[_to] = _balance
        self._update_liquidity_limit(_to, _balance, _supply)
