...
import LiquidityGaugeWallet as LiquidityGaugeWallet
...
wallet_library: public(address)
balanceOf: public(HashMap[address, uint256])
walletOf: public(HashMap[address, address])
...
def deposit(_value: uint256, addr: address = msg.sender):
    ...
    if _value != 0:
        ...
        ERC20(self.lp_token).transferFrom(msg.sender, _wallet, _value)
        LiquidityGaugeWallet(_wallet).deposit(_value)
...
def withdraw(_value: uint256):
    ...
    if _value != 0:
        ...
        LiquidityGaugeWallet(self.walletOf[msg.sender]).withdraw(_withdraw_value)
...
def liquidate(_target: address, _liquidator: address) -> bool:
    ...
    _current_balance: uint256 = ERC20(self.atoken).balanceOf(msg.sender)
    _expected_balance: uint256 = self.balanceOf[_target]

    assert _current_balance < _expected_balance, "Nothing to liquidate"
    _liquidation_amount: uint256 = _expected_balance - _current_balance

    _totalSupply: uint256 = self.totalSupply - _liquidation_amount
    self.totalSupply = _totalSupply
    self.balanceOf[_target] = _current_balance

    self._update_liquidity_limit(_target, _current_balance, _totalSupply)

    # TODO how to reward the liquidator?
